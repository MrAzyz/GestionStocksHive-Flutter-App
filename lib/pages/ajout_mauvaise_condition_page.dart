...
class AjoutMauvaiseConditionPage extends StatefulWidget {
  const AjoutMauvaiseConditionPage({Key? key}) : super(key: key);

  @override
  State<AjoutMauvaiseConditionPage> createState() =>
      _AjoutMauvaiseConditionPageState();
}

class _AjoutMauvaiseConditionPageState extends State<AjoutMauvaiseConditionPage> {
  ...
  Future<void> _saveProduit() async {
    if (!_formKey.currentState!.validate() || _selectedImagePath.isEmpty) {
        ...
        SnackBar(
          content: const Row(
            children: [
              ...
              SizedBox(width: 12),
              Expanded(
                child: Text('Veuillez remplir tous les champs et sélectionner une image.'),
              ),
            ],
          ),
    ...
    final imagesDir = Directory('${appDocDir.path}/images_mauvaise');
    if (!await imagesDir.exists()) {
      await imagesDir.create(recursive: true);
    }
    final fileName = path.basename(_selectedImagePath);
    final newImagePath = path.join(imagesDir.path, '${DateTime.now().millisecondsSinceEpoch}_$fileName');
    await File(_selectedImagePath).copy(newImagePath);


    final produit = ProduitMauvaiseCondition(
    ...
    final box = Hive.box<ProduitMauvaiseCondition>('mauvaiseConditionBox');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text(
          'Ajouter Produit Mauvaise Condition',
          style: TextStyle(
                      ...
                      const Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Ajoutez un produit en mauvaise condition',
                              style: TextStyle(
            ...
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  icon: const Icon(Icons.photo_library, color: Color(0xFFE74C3C)),
                  label: Text(
